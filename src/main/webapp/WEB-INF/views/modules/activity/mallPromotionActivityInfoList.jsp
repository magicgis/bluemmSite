...
<head>
	...
	<script type="text/javascript">
		...
		function resetBtn(){
		    ...
		    $("#sendCouponType").find("option[value='']").attr("selected",true);
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/activity/mallPromotionActivityInfo/">优惠券活动列表</a></li>
		<shiro:hasPermission name="activity:mallPromotionActivityInfo:edit"><li><a href="${ctx}/activity/mallPromotionActivityInfo/form">优惠券活动添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="mallPromotionActivityInfo" action="${ctx}/activity/mallPromotionActivityInfo/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>活动大类：</label>
				<form:select path="activityTheme" style="width:177px;" id="activityTheme">
			...
			<li><label>券发放方式：</label>
				<form:select path="sendCouponType" style="width:177px;" id="sendCouponType">
					<form:option value="" label="--请选择--"/>
					<form:options items="${fns:getDictList('activity_send_coupon_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th align="center">发券方式</th>
				<th align="center">状态</th>
				<th align="center">开始时间</th>
				<th align="center">结束时间</th>
				<th align="center">操作人</th>
				<th align="center">操作时间</th>
				<shiro:hasPermission name="activity:mallPromotionActivityInfo:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="mallPromotionActivityInfo" varStatus="status">
			<tr>
				...
				<td align="center">${fns:getDictLabel(mallPromotionActivityInfo.sendCouponType, 'activity_send_coupon_type', '')}</td>
				...
				<shiro:hasPermission name="activity:mallPromotionActivityInfo:edit"><td>
				<c:if test="${mallPromotionActivityInfo.status == 1}">
					<a href="${ctx}/activity/mallPromotionActivityInfo/modify?id=${mallPromotionActivityInfo.activityId}">编辑</a>
					</c:if>
    				<a href="${ctx}/activity/mallPromotionActivityInfo/form?id=${mallPromotionActivityInfo.activityId}">查看</a>
    				<c:if test="${mallPromotionActivityInfo.status == 1 ||mallPromotionActivityInfo.status == 2 }">
					<a href="${ctx}/activity/mallPromotionActivityInfo/delete?id=${mallPromotionActivityInfo.activityId}" onclick="return confirmx('确认要取消该优惠券活动吗？', this.href)">取消</a>
					</c:if>
