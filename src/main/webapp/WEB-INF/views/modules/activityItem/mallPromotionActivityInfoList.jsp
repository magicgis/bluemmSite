...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/activityItem/mallPromotionActivityInfo/">商品活动列表</a></li>
		<shiro:hasPermission name="activityMJS:mallPromotionActivityInfo:edit"><li><a href="${ctx}/activityItem/mallPromotionActivityInfo/form">商品活动添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="mallPromotionActivityInfo" action="${ctx}/activityItem/mallPromotionActivityInfo/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<form:hidden path="pageCount" value="${page.totalPage}"/>
		<ul class="ul-form">
			<li><label>活动编码：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="mallPromotionActivityInfo" varStatus="status">
			<tr>
				...
				<shiro:hasPermission name="activityItem:mallPromotionActivityInfo:edit"><td>
				<c:if test="${mallPromotionActivityInfo.status == 1}">
    				<a href="${ctx}/activityItem/mallPromotionActivityInfo/modify?id=${mallPromotionActivityInfo.activityId}">编辑</a>
    				</c:if>
    				<a href="${ctx}/activityItem/mallPromotionActivityInfo/info?id=${mallPromotionActivityInfo.activityId}">查看</a>
    				<c:if test="${mallPromotionActivityInfo.status == 1 ||mallPromotionActivityInfo.status == 2 }">
					<a href="${ctx}/activityItem/mallPromotionActivityInfo/delete?id=${mallPromotionActivityInfo.activityId}" onclick="return confirmx('确认要取消该商品活动吗？', this.href)">取消</a>
					</c:if>
