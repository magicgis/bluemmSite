...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/hb/mallWxRedpacketApply/">红包申请列表</a></li>
	...
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/hb/mallWxRedpacketApply/import" enctype="multipart/form-data" class="form-search"
			...
			<input type="hidden" id="merchantNoCode" name="merchantNoCode" />
			<input type="hidden" id="webchatPublicNoCode" name="webchatPublicNoCode" />
			<input type="hidden" id="merchantNameCode" name="merchantNameCode" />
			<input type="hidden" id="providerCode" name="providerCode" />
			<input type="hidden" id="applyidCode" name="applyidCode" />
		</form>
	</div>
...
<!-- 					<strong>数据导入</strong> -->
					...
					<td colspan="3">
<!--						<input id="btnImport" class="btn btn-primary" type="button" value="导入数据" disabled="disabled"/> 
						<input class="btn" type="button" value="清空数据" onclick="clearData();" disabled="disabled"/>
		...
		<c:if test="${mallWxRedpacketApply.applystatus > 2}">
		<fieldset title="审核信息">
			<legend>审核信息</legend>
			<table style="width: 95%;">
				<tr>
				...
				<tr>
					<td width="8%"><strong>审核意见1</strong></td>
					<td align="left">
						<form:textarea path="backReason" cols="100" rows="3" readonly="true"
		...
		<script type="text/javascript">
			function reback(){
				...
				location.href='${ctx}/hb/mallWxRedpacketApply/list?type='+fromType;
			}
		</script>
	</form:form>
</body>
